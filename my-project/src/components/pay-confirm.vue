    ...
    <section class="category-list-box" v-infinite-scroll="loadMore" infinite-scroll-disabled="busy" infinite-scroll-distance="10">
        ...
        <div class="media" v-for="(buycarObj, index) in buycarListMap.list">
            ...
            <section class="action-box">
                <i class="iconfont icon-yuanquanjianhao" v-on:click="reduceBuycar(buycarObj.relateId,index,buycarObj.status)"></i>
...
export default {
        ...
        methods: {
            ...
            loadMore: function() {
                ...
                var recordId = _self.buycarListMap.pmap[buycarObj.relateId].recordId;
            ...
            },reduceBuycar: function(relateId,index,status){
                var _self = this;
                var buycarObj = _self.buycarListMap.list[index];
                ...
                if(number >= 1){
                    setTimeout(function(){
                        //增加货物到购物车
                        var reducebuycarMapPara = {"api_u_key":getCookie("api_u_key"),"recordId":recordId,"status":status,"number":(number - 1)};
                        commonAjax("/api/buycar/updatebuycar.xhtml",reducebuycarMapPara,"post",reducebuycarMapFunc);
                    }, 100);
                }
                ...
                function payBuyCarMapFunc(data){
                    data = JSON.parse(data);
                    if(data.isSuccess){
                        console.log("===" + data.data.tradeNo);
                        //调用/api/pay/mppay.xhtml 接口进行判断
                        var mppayPara = {"api_u_key":getCookie("api_u_key"),"tradeNo": data.data.tradeNo};
                        commonAjax("/api/pay/mppay.xhtml",mppayPara,"post",function(data){
                            console.log("---------=======----------");
                            console.log(data);
                            data = JSON.parse(data);
                        });
                ...
                setTimeout(function(){
                    ...
                    var recordId = 33;
                    //增加货物到购物车
                    var payBuyCarMapPara = {"api_u_key":getCookie("api_u_key"),"addressId": recordId};
