...
<template>
  <div id="recharge">
     ...
     <section class="recharge-box-title">
         <p>充值付款，享受更多折扣</p>
         <div>账户余额（元） <span v-show="memberInfo.amount">￥{{memberInfo.amount}}</span><span v-show="!memberInfo.amount">￥0</span>
         </div>
     </section>
     <ul class="recharge-list">
         <li v-for="discount in discountList"><span>￥{{discount.chargeAmount}}</span>充值金额￥{{discount.disCountAmount}}元 <button class="btn btn-success btn-sm" v-on:click="payCharger(discount.recordId)">立即充值</button></li>
     </ul>
  </div>
</template>
